...
const SERVICE_FAQS: Record<string, { q: string; a: string }[]> = {
  ...
  "shockwave-therapy": [
    { q: "При каких заболеваниях помогает УВТ?", a: "Ударно-волновая терапия наиболее эффективна при пяточной шпоре и подошвенном фасциите, кальцифицирующем тендините плеча, эпикондилите локтя, тендинопатии ахиллова сухожилия, трохантерите и миофасциальных болях. При артрозе применяется как дополнение к основному лечению." },
    { q: "Сколько сеансов УВТ нужно на курс?", a: "Стандартный курс — 3–7 процедур с интервалом 5–10 дней. При свежем подошвенном фасциите бывает достаточно 3–4 сеансов, при кальцифицирующем тендините плеча обычно требуется 5–7. Точное количество врач определяет после УЗИ." },
    { q: "Больно ли делать ударно-волновую терапию?", a: "Процедура терпима и не требует анестезии. Ощущения описывают как ритмичные постукивания с ломотой в глубине тканей. Врач наращивает мощность постепенно и ориентируется на вашу обратную связь, поэтому воздействие остаётся комфортным." },
    { q: "Есть ли противопоказания к УВТ?", a: "Да. УВТ противопоказана при онкологии, беременности, нарушениях свёртываемости крови, приёме антикоагулянтов без коррекции, наличии кардиостимулятора, острой инфекции или гнойном процессе в зоне воздействия. Перед курсом врач проводит осмотр." },
  ],
  blockades: [
...
const SEO_META: Record<string, { title: string; description: string }> = {
  ...
  "shockwave-therapy": {
    title: "Ударно-волновая терапия в Новосибирске — УВТ пяточной шпоры, суставов | Ваш Ортопед",
    description: "Ударно-волновая терапия (УВТ) при пяточной шпоре, тендините плеча, эпикондилите и болях в суставах в Новосибирске. Без проколов и наркоза. Клиника Ваш Ортопед, Есенина 67. Запись: +7 999 464 91 94.",
  },
  blockades: {
