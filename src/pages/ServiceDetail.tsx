...
const SERVICE_FAQS: Record<string, { q: string; a: string }[]> = {
  ...
  "svf-therapy": [
    { q: "Что такое SVF-терапия простыми словами?", a: "Это лечение сустава клетками, выделенными из собственной жировой ткани пациента. У вас берут небольшой объём жира через микропрокол, выделяют концентрат регенеративных клеток (стромально-васкулярную фракцию) и вводят в больной сустав под контролем УЗИ. Эти клетки снимают хроническое воспаление и запускают восстановление хряща." },
    { q: "Чем SVF-терапия отличается от PRP?", a: "PRP-терапия использует плазму крови, обогащённую тромбоцитами, — это концентрат факторов роста. SVF содержит живые регенеративные клетки из жировой ткани и обладает более мощным и длительным восстановительным потенциалом. PRP чаще применяют при начальных стадиях артроза, SVF — при 2–3 стадии с выраженными изменениями хряща." },
    { q: "Можно ли с помощью SVF избежать замены сустава?", a: "При артрозе 1–3 стадии SVF-терапия во многих случаях позволяет надолго отсрочить или полностью избежать эндопротезирования, убрав боль и восстановив функцию сустава. При 4 стадии, когда хрящ полностью разрушен, метод неэффективен — в этой ситуации мы честно рекомендуем консультацию хирурга." },
    { q: "Через сколько появится результат и сколько он держится?", a: "SVF работает постепенно: первые изменения через 3–4 недели, устойчивый эффект — к 2–3 месяцу, максимальный — к 6 месяцам. Длительность результата составляет от 18 месяцев до нескольких лет в зависимости от стадии артроза, возраста и массы тела." },
    { q: "Безопасна ли процедура? Есть ли риск отторжения?", a: "Риск отторжения отсутствует полностью — используется собственная ткань пациента, а не донорский материал. Процедура выполняется в стерильных условиях. Противопоказания: активная онкология, острые инфекции, тяжёлые нарушения свёртываемости крови, беременность, обострение аутоиммунных заболеваний." },
  ],
  "shockwave-therapy": [
...
const SEO_META: Record<string, { title: string; description: string }> = {
  ...
  "svf-therapy": {
    title: "SVF-терапия в Новосибирске — стромально-васкулярная фракция, лечение артроза | Ваш Ортопед",
    description: "SVF-терапия (стромально-васкулярная фракция) при артрозе коленного и тазобедренного суставов в Новосибирске. Клеточное лечение без операции, альтернатива эндопротезированию. Клиника Ваш Ортопед, Есенина 67. Запись: +7 999 464 91 94.",
  },
  "shockwave-therapy": {
